...
mod test {
    ...
    fn line_string() -> Result<()> {
        ...
        assert!(
            read_geojson_geom(&mut geojson.as_bytes(), &mut WktWriter::new(&mut wkt_data, WktDialect::Wkt)).is_ok()
        );
    ...
    fn feature_collection() -> Result<()> {
        ...
        let mut wkt_data: Vec<u8> = Vec::new();
        assert!(read_geojson_fc(geojson.as_bytes(), &mut WktWriter::new(&mut wkt_data, WktDialect::Wkt)).is_ok());
        let wkt = std::str::from_utf8(&wkt_data).unwrap();
