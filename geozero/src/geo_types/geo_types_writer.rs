...
mod test {
    ...
    fn geometry_collection() {
        ...
        use geo_types::{Geometry, GeometryCollection, line_string, point};
    ...
    fn nested_geometry_collections() {
        use crate::wkt::Wkt;
        let wkt = Wkt(
            "GEOMETRYCOLLECTION(POINT(1 2),GEOMETRYCOLLECTION(LINESTRING(1 2,3 4), MULTIPOINT(1 2, 3 4, 5 6)),POINT(7 8))",
        );
        let actual = wkt.to_geo().unwrap();

        use geo_types::{Geometry, GeometryCollection, MultiPoint, line_string, point};
